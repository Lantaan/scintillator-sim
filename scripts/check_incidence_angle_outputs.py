#!/usr/bin/env python3
"""Lightweight sanity checks for archived incidence-angle runs.

This script intentionally uses only the Python standard library so it can run
inside the project WSL environment between long simulation runs.
"""
...
def parse_angle(case_dir: Path) -> float:
...
def absorption_metrics(path: Path, width_mm: float, length_mm: float) -> dict[str, float]:
    ...
    bins = choose_bins(n)
    x_lo = min(x_min, -width_mm / 2.0)
    x_hi = max(x_max, width_mm / 2.0)
    y_lo = min(y_min, -length_mm / 2.0)
    y_hi = max(y_max, length_mm / 2.0)
...
def check_case(case_dir: Path) -> dict[str, object]:
    ...
    metrics: dict[str, float] = {
        "angle_deg": parse_angle(case_dir),
...
def case_dirs(run_root: Path, angle: str | None) -> list[Path]:
    ...
    return sorted((p for p in run_root.glob("angle_*deg") if p.is_dir()), key=parse_angle)
...
def main() -> None:
    ...
    for case_dir in case_dirs(run_root, args.angle):
        ...
        rows.append(row)
        angle = row["angle_deg"]
        with (out_dir / f"angle_{angle_slug(angle)}deg_sanity.json").open("w", encoding="utf-8") as f:
            json.dump(row, f, indent=2, sort_keys=True)
        flags = row.get("sanity_flags", [])
        if flags:
            print(f"[sanity] angle={angle:g} flags: {'; '.join(flags)}")
        else:
            print(f"[sanity] angle={angle:g} ok")

    if args.write_summary or args.angle is None:
